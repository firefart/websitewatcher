...
```bash
task build        # fmt + vet + build binary
...
**Key packages:**
- `internal/config/` — JSON config loading via koanf, validation via go-playground/validator. Validates cron expressions, emails, URLs, JQ syntax at startup. Conflicting options (e.g., `pattern` + `extract_body`) are rejected.
...
Config is JSON (`-config` flag required). See `config.sample.json` for all options. Key watch-level options:
...
`pattern` and `extract_body` cannot both be set on the same watch.
